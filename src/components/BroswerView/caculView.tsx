...
interface IProps {
  children?: ReactNode
  handler: Promise<(...args: any) => any>
  args?: any
}

const CaculView: FC<IProps> = ({ handler, args = [] }) => {
  return (
    <BrowserOnly>
      {() => {
        ...
        return (
          <BrowserWindow url={location.href}>
            <button className={clsx(styles.button)} onClick={handleClick}>
              ...
              <p className={clsx(styles.text)}>Click me</p>
            </button>
            ...
            <div style={{ display: 'flex' }}>
              ...
              <Card className={clsx(styles.card)}>{result}</Card>
            </div>
          </BrowserWindow>
        )
      }}
